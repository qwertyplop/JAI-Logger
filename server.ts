...
// Catch-all route to avoid path-to-regexp wildcards issues; manually filter proxy requests
app.all("*", async (req, res) => {
  ...
  // segments[0] === "proxy", segments[1] === sessionId
...
// Global middleware to catch proxy requests without path-to-regexp wildcards
app.use(async (req, res, next) => {
  const path = req.path;
  if (!path.startsWith("/proxy/")) return next();
